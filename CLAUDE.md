...
- `make build` — builds `bin/shedd` (+ guest agent) and codesigns it. Plain
...
append the pubkey to `~/.local/share/shed/authorized_keys`, then
`ssh -o IdentityAgent=none -o IdentitiesOnly=yes -i <key> shed ls`.
...
- Kernel: Kata static arm64 `vmlinux-6.18.15-186` (kata 3.28.0), cached at
  `~/Library/Caches/shed/kernel/3.28.0/Image`; sha pinned in internal/kernel.
...
- The default image "exeuntu" is baked locally on first use by
  ...
  ~/Library/Caches/shed/base/exeuntu-<hash>.img with a .json sidecar.
